...
def group_post():
    ...
    sleep(5)
    for i in    groups_links_list:
    # for i in range(len(groups_links_list)):
        try:
            link = 'https://www.facebook.com/groups/' + groups_links_list[i]
            driver.get(link)
            sleep(6)
            print("group opened",link)
            # driver.find_element_by_xpath("//*/div/div[1]/div/div[3]/div/div/div[1]/div[1]/div[2]/div/div/div/div[4]/div/div/div/div/div[1]/div[1]/div/div/div/div[1]/div").click()
            driver.find_element_by_xpath("//*/div/div[1]/div/div[3]/div/div/div[1]/div[1]/div[4]/div/div/div/div/div[1]/div[1]/div/div/div/div[1]/div").click()
            # driver.find_element_by_xpath("//span[contains(text(),'Create a public post…')]").click()
            print("clicked on create post")
            sleep(3)
